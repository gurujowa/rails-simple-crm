class ConvertLeadFromSonouchi < ActiveRecord::Migration
  def up
    add_column :leads, :campaign, :string
    add_column :leads, :campaign_detail, :string
    LeadHistory.delete_all
    Lead.delete_all
    companies = Company.where(active_st: [:pending, :active_c, :active_b, :active_a])
    companies.each do |c|
      lead = Lead.new
      lead.name = c.name
      lead.tel = c.tel
  ...
  def down
    remove_column :leads, :campaign
    remove_column :leads, :campaign_detail
    LeadHistory.delete_all
    Lead.delete_all
  end
end
